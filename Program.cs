        ...
        static void MainAction(StartupOptions options)
        {
            async Task MainActionAsync()
            {
                ...
                while (true)
                {
                    ...
                    ElementHandle[]? items = (ElementHandle[])await page.QuerySelectorAllAsync(query);

                    if (items.Length == 0)
                    {
                        string msgQuery = QueryHelper.AllChatMessages();
                        ElementHandle[]? msgs = (ElementHandle[])await page.QuerySelectorAllAsync(msgQuery);
